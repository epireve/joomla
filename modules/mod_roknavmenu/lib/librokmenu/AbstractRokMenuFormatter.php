...
abstract class AbstractRokMenuFormatter implements RokMenuFormatter
{
    ...
    protected function _default_format_menu(&$menu)
    {
        ...
        if (!array_key_exists('always_show', $this->args) && $start==0)
        {
            $always_show_menu = true;
        }
        elseif (array_key_exists('always_show', $this->args))
        {
             $always_show_menu = $this->args['always_show'];
        }
        ...
        if (!$this->args['showAllChildren'])
        {
            if ($menu->hasChildren())
            {
                if (count($this->active_branch) == 0 && !$always_show_menu)
                {
                    $menu->removeLevel($start - 1);
                }
                else
                {
